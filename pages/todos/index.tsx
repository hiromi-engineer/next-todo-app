  ...
  return (
    <>
      ...
      <select name="order" value={order} onChange={changeOrder}>
        {orderes.map((item, index) => <option key={index} value={item.value}>{item.desc}</option>)}
      </select>
      <ul>
        { todos.map((todo, index) => {
            return (
              (filter.status === '全て' || todo.status === filter.status) && <li key={index}><ListItem todo={todo} /></li>
            );
          })
        }
      </ul>
